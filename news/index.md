...
- **lme4 reference fits are skipped on the Windows CI runner.** lme4
  ...
  `glmer()`/[`vcov()`](https://rdrr.io/r/stats/vcov.html) on GitHub’s
  Windows runner, even when lme4 is compiled from source there — an
  upstream bug no R-level guard can catch. `ref_fit()` (tests) and the
  validation harness’s reference-fit wrapper now skip lme4 reference
  fits on that runner only; the cross-walks still run on macOS, all
  Linux CI platforms, and everywhere locally. gllammr’s own fits are
  unaffected.
