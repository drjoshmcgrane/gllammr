...
**Key insight:** for the logit link, marginal probabilities are pulled toward 0.5 relative to conditional probabilities when the random-effect variance is substantial, because the inverse logit flattens in the tails.
...
**Use case:** automatic item generation, item bank calibration, and predicting item performance before pilot testing.
...
1. **Cluster-specific inference**: you want the expected outcome for a specific cluster (a particular school, hospital, or person)
...
1. **Population-level inference**: expected outcome averaged across all clusters
...
5. **Reporting to non-technical audiences**: marginal predictions often have a simpler interpretation
...
For more information, see `?predict.gllamm` and the related predict methods.
