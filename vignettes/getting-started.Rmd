...
Every model class in the package runs through `gllamm()`; the family
object selects the model:

| Model class | Family / call shape | Vignette |
|---|---|---|
| GLMMs (crossed/nested REs, slopes) | `gaussian()`, `binomial()`, `poisson()`, `Gamma()` | `multilevel-glmm` |
| Ordinal (5 links), multinomial | `ordinal(link)`, `multinomial()` | `multilevel-glmm` |
| IRT (Rasch ... NRM) | `gllamm(resp, family = irt(model))` | `irt-models` |
| Explanatory IRT | `family = eirt(item_data, ~ ...)` | `explanatory-irt` |
| Multilevel IRT | `irt()` + `random = ~ (1 | group)` | `multilevel-irt` |
| DIF (screening + confirmatory) | `dif_test()`, `dif_irt()` | `dif-analysis` |
...
| Mixed responses, survival, ranks, NPML, AGHQ | `mixed_response()`, `survival_family()`, `ranking()`, `integration = npml(k)/aghq(k)` | `advanced-features` |
